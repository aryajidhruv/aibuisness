...
// 🌟 SIDEBAR LINKS UPDATE: Workflow aur Dialer dono items add kar diye hain bhai
const NAV_ITEMS = [
    ...
    <div className="min-h-screen bg-[#080C14] text-white flex">
      ...
      <aside className="w-56 bg-[#0F1623] border-r border-white/5 fixed top-0 left-0 bottom-0 flex flex-col z-40 hidden md:flex">
        ...
        <nav className="flex-1 py-3">
          {NAV_ITEMS.map(item => (
            <div
              key={item.label}
              // 🌟 NAVIGATION FLOW TRIGGER: click par target routes dynamic handle honge
      ...
      <main className="flex-1 md:ml-56 p-6 md:p-8">
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          ...
          {/* Quick Shortcuts Trigger Grid */}
          <div className="flex gap-3">
            <button
              onClick={() => navigate('workflow')}
              className="bg-slate-800 hover:bg-slate-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition-all"
            >
              🔀 Build Workflow
            </button>
            <button
